...
def main(argv=None):
    ...
    if a.output is None: p.error("--output is required for execution")
    if a.task!="STSB" and bundle is None: p.error("--data is required for this task")
    if plan["pca_components"] and calibration is None: p.error("--calibration is required for reduced dimensions")
