"""Offline configuration and runner wiring tests; no pretrained model execution."""
import json
...
def test_required_matrix():
    ...
    assert all(spec["model_id"] is None for name, spec in MODEL_SPECS.items() if name != "Phi4-mini")
...
def test_future_entries_are_explicitly_unimplemented():
    plan = make_plan(model="Qwen3-8B", task="SciFact", train_count=3000)
    assert not plan["executable"] and plan["model_id"] is None
    assert plan["pca_components"] == 2048 and plan["blockers"]


def test_dry_run_without_site_packages_or_gpu(tmp_path):
    # -S proves dry-run works without torch, transformers, numpy, or datasets.
...
def test_future_execution_rejected_before_imports(tmp_path):
    result = subprocess.run([sys.executable, "-S", str(ROOT / "run_pilot.py"),
                             "--model", "Qwen3-8B", "--train-sentences", "3000"],
                            cwd=tmp_path, capture_output=True, text=True)
    assert result.returncode != 0
    assert "Execution unavailable" in result.stderr
...
def test_cache_native_width_is_explicit(tmp_path):
    ...
    assert hit
    with pytest.raises(ValueError):
        cached_embeddings(tmp_path, {}, ["a"], encode, native_dimension=32)


@pytest.mark.parametrize("dimensions,expected_components", [
    (None, 768), ([3072, 1536, 768, 384], 1536), ([3072], 0),
])
def test_runner_wiring_keeps_training_and_evaluation_separate(monkeypatch, tmp_path, dimensions, expected_components):
    ...
    calls = {}
    train_texts, eval_texts = ["training only"], ["left", "right"]
    ...
    monkeypatch.setitem(sys.modules, "huggingface_hub", SimpleNamespace(HfApi=lambda: SimpleNamespace(
        model_info=lambda *a, **k: SimpleNamespace(sha="model-sha"),
        dataset_info=lambda *a, **k: SimpleNamespace(sha="data-sha"))))
    monkeypatch.setitem(sys.modules, "torch", SimpleNamespace(manual_seed=lambda n: None,
        cuda=SimpleNamespace(manual_seed_all=lambda n: None, synchronize=lambda: None, max_memory_allocated=lambda: 0),
    ...
    monkeypatch.setattr(model, "PhiEncoder", lambda *a: SimpleNamespace(dtype="test",
        validate_batching=lambda: {}, encode=lambda texts: None,
        tokenizer=lambda texts, **k: {"input_ids": [[1] for _ in texts]}))
    def selection(*a, **k):
    ...
    def cached(directory, identity, texts, encode):
        return (train_x if texts == train_texts else eval_x), True
    monkeypatch.setattr(core, "cached_embeddings", cached)
    def fitting(array, max_dim, seed):
        assert array is train_x  # Never the evaluation array.
        assert max_dim == expected_components
        calls["fit"] = True
    ...
    def projection(array, pca, dimension):
        assert array is eval_x and dimension <= expected_components
        calls.setdefault("projected", []).append(dimension)
        return np.ones((2, dimension))
    monkeypatch.setattr(core, "project", projection)
    monkeypatch.setattr(core, "score_pairs", lambda *a: (0.0, np.zeros(1)))
    def saving(output, rows, metadata):
        output.mkdir()
        calls["rows"], calls["metadata"] = rows, metadata
    monkeypatch.setattr(run_pilot, "save_outputs", saving)
    argv = ["run_pilot.py", "--output", str(tmp_path / "result")]
    if dimensions is not None:
        argv += ["--dimensions", *map(str, dimensions)]
...
def test_four_dimension_export_and_plot_ticks(monkeypatch, tmp_path):
    ...
    # Temporary serialization fixture, not a measured experiment.
...
def test_legacy_native_cache_key_is_preserved(tmp_path):
    ...
    array = np.ones((1, 3072), dtype=np.float32)
    cached_embeddings(tmp_path, identity, texts, lambda _: array)
